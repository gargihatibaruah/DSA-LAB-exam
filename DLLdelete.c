...
void delete_node(int pos) {
    ...
    if (pos == 1) {  // Delete at beginning
        ...
        free(temp);
    } else if (pos == 0x7FFFFFFF) {  // Delete at end
        while (temp->next) temp = temp->next;  // Traverse to the last node
        if (temp->prev) temp->prev->next = NULL;  // Update the previous node
        else header = NULL;  // If it's the only node in the list
        free(temp);
    } else {  // Delete at a specific position
